...
from src.components.model_trainer import RandomForest, HyperparameterTuner,  XGBoost
import pandas as pd
import numpy as np
...
def train_model(X_train:np.ndarray, y_train:pd.Series)-> ClassifierMixin:
    ...
    try:
        ...
        if config.model =="RandomForestClassifier":
            ...
            logging.info("model is randomforest")
            ...
            model_class = XGBoost()
            base_model = model_class.base_model()
            logging.info("model is XGBClassifier")
